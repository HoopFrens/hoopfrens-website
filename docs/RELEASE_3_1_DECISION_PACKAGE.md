...
This is a historical decision proposal. The explicit September 6, 2026 Founder instruction authorizes Release 4.1 Governed Content Intelligence, EO-055 through EO-059, and takes precedence for that batch. The decision table below is retained as historical context, not a current approval ledger or an additional permission gate. The current [Decision Log](./DECISIONS.md) and [re-entry audit](./RELEASE_4_1_REENTRY_AUDIT.md) distinguish approved scope from unresolved runtime settings. In particular, automatic Research Package creation is explicitly authorized after Founder initiation; autonomous CIO behavior remains excluded. The table remains a historical proposal. The subsequent Founder decision supersedes its budget entries with $0.50/request and $10/month; current implementation fixes gpt-4.1-mini routing as an engineering choice. Historical retention durations remain unconfirmed. Refer to the current decision log for runtime status.
