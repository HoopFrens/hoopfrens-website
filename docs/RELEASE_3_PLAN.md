...
Add governed AI reasoning to Headquarters without weakening the deterministic workflow, authorization model, Founder control, or auditability established in Release 2.

This document is an architecture plan only. Release 3 implementation has not started. Every capability remains subject to the Founder decisions in [`RELEASE_3_1_DECISION_PACKAGE.md`](./RELEASE_3_1_DECISION_PACKAGE.md), an approved Engineering Order, capability-specific threat modeling, and the four-gate release process.
...
### Release 3.1 — AI Foundation and Governance
...
- Founder approval of all thirteen Release 3.1 decisions before implementation.
...
- Release 3.2 cannot begin implementation until 3.1 is released and its controls are verified in the deployed environment.
...
The 3.1 gateway receives only authorized, minimal project context. The reasoning capability cannot access Firestore directly and cannot receive mutation tools.
...
- Complete the four gates independently from 3.1.
...
Each editorial role is a separate capability contract behind the 3.1 gateway. Roles receive only approved, project-scoped artifacts and expose no direct persistence or publishing tool. Deterministic services own version creation and persistence after approval.
...
- Implementation before Release 3.1 governance is complete and released.
...
- Release 3.1 governance must already be released and independently verified.
...
Each capability requires its own approved Engineering Order and bounded branch. A later capability may depend on a released earlier capability, but planning approval does not authorize implementation. Release 3.5 remains deferred until the governance foundation is complete. No capability may weaken the Release 2 lifecycle, authorization, persistence, Founder approval, or audit standards.
